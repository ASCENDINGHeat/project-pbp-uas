<script lang="ts">
	...
	function handleLogin() {
		if (email && password) {
			// 1. Simpan status "sudah login" ke memori browser (Local Storage)
			localStorage.setItem("userLoggedIn", "true");
			localStorage.setItem("userName", email); // Simpan nama user
			
			// 2. Arahkan paksa ke halaman pengguna
...
<div class="overlay">
	<div class="login-container">
		<div class="login-card" role="dialog" aria-modal="true" aria-label="Login">
			...
			<p class="footer-text">Belum punya akun? <a href="/register" on:click|preventDefault={() => goto('/register')}>Daftar di sini</a></p>
...
<style>
	...
	.back-link { display: block; margin-top: 20px; color: #999; text-decoration: none; font-size: 0.85rem; cursor: pointer; }
</style>
